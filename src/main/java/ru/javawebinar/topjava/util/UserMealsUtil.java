...
//        .toLocalTime();
    ...
    public static List<UserMealWithExceed> getFilteredWithExceededStream(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> mapForExceed = new HashMap<>();
        mealList.forEach(um -> mapForExceed.put(um.getDateTime().toLocalDate(), mapForExceed.getOrDefault(um.getDateTime().toLocalDate(), 0)
                + um.getCalories()));
        ...
        return mealList.stream()
                ...
                .map(temp -> new UserMealWithExceed(temp.getDateTime(), temp.getDescription(), temp.getCalories(),
                        mapForExceed.get(temp.getDateTime().toLocalDate()) > caloriesPerDay))
